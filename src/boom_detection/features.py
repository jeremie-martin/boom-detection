...
def _coverage_from_angles(angles: np.ndarray, n_bins: int = 36) -> np.ndarray:
    """
    Compute birthday-corrected coverage of angular bins.

    This is the key predictive signal - high coverage means pendulums
    are spread across many angular bins (chaos/boom signature).
    ...
    Returns:
        Shape (frames,) - coverage values in [0, 1+]
    """
    n_pendulums = angles.shape[1]
    counts = _angular_histogram(angles, n_bins)
